...
export function Step4Dates({ tripData, updateTripData, onNext, registerCommit }: Step4Props) {
  ...
  const [startMonth, setStartMonth] = useState<Date>(() => {
    const d = tripData?.startDate;
    if (d && d.year && d.month) return new Date(d.year, d.month - 1, 1);
    return new Date();
  });
  ...
  const [endMonth, setEndMonth] = useState<Date>(() => {
    const d = tripData?.endDate;
    if (d && d.year && d.month) return new Date(d.year, d.month - 1, 1);
    const sd = tripData?.startDate;
    if (sd && sd.year && sd.month) return new Date(sd.year, sd.month - 1, 1);
    return new Date();
  });
  ...
  }, [canConfirm, effectiveZone, endInput, registerCommit, startInput, updateTripData]);
