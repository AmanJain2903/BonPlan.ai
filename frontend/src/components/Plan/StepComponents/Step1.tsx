  ...
  <div className="absolute inset-0 flex items-center px-6 gap-3">
    <div className="flex -space-x-3">
      {[0, 1, 2, 3].map((i) => (
        <div
          key={i}
          className="h-12 w-12 rounded-full bg-purple-400/15 border-2 border-carbon/60 flex items-center justify-center"
          style={{ animation: `fade-in 400ms ease-out ${i * 120}ms both`, zIndex: 4 - i }}
        >
          <User size={18} className="text-purple-400/70" />
        </div>
      ))}
    </div>
    <div className="flex-1 flex flex-col items-center gap-1.5">
      <div className="flex gap-1">
        {[0, 1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="h-2 w-2 rounded-full bg-purple-400/30"
            style={{ animation: `pulse 1.5s ease-in-out ${i * 0.2}s infinite` }}
          />
        ))}
      </div>
      <div className="h-3 w-24 rounded-full bg-purple-400/10 animate-[pulse_2s_ease-in-out_infinite]" />
    </div>
  </div>
);
...
const THEME_MAP: Record<string, any> = {
  ...
  squad: {
    borderColorHover: 'hover:border-purple-400/30',
    shadowHover: 'hover:shadow-[0_0_60px_rgba(168,85,247,0.08)]',
    radialGradient: 'radial-gradient(circle at 70% 30%, rgba(168,85,247,0.20), rgba(168,85,247,0) 58%)',
    linearGradient: 'linear-gradient(90deg, rgba(168,85,247,0), rgba(168,85,247,0.10), rgba(168,85,247,0))',
...
export function Step1PlanningStyle({ hoveredTip, onTipChange, onSelect }: Props) {
  ...
  const stepData = COMMON_PLAN_STEPS.find((step) => step.id === 'planning-style');
